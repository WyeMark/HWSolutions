report 50003 "H&W Purchase Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/reports/PurchaseOrder.rdlc';
    Caption = 'H&W Purchase Order';
        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(Infomation; BuildInformation)
                    {
    ...
    local procedure BuildInformation(): Text
    var
        ShipmentRec: Record "PVS Job Shipment";
        CRLF: Text[2];
        OutTxt: Text;
        TempTxt: Text;
        HW001: Label 'Quantity %1 - Address: %2';
        HW002: Label 'Shipping Addresses:';
    begin
        CRLF[1] := 13;
        CRLF[2] := 10;

        ShipmentRec.Reset();
        ShipmentRec.SetRange(ID, "Purchase Header"."PVS ID");
        ShipmentRec.SetRange("Optional Field 3", 'Yes');
        if ShipmentRec.FindSet() then begin
            OutTxt := "Purchase Header".GetFullDocTypeTxt() + CRLF + CRLF + HW002 + CRLF;
            repeat
                OutTxt += CRLF;
                TempTxt := ShipmentRec.Name;
                TempTxt += ' ' + ShipmentRec."Name 2";
                TempTxt += ' ' + ShipmentRec.Address;
                TempTxt += ' ' + ShipmentRec."Address 2";
                TempTxt += ' ' + ShipmentRec."Post Code" + ' ' + ShipmentRec.City;
                TempTxt += ' ' + ShipmentRec.Contact;
                TempTxt += ' ' + ShipmentRec."Port/Ramp";
                OutTxt += StrSubstNo(HW001, ShipmentRec."Promised Qty.", DelChr(CopyStr(TempTxt, 1, 250), '<>', ' '));
            until ShipmentRec.Next() = 0;
        end else
            OutTxt := "Purchase Header"."PVS Order No.";
    end;
}
